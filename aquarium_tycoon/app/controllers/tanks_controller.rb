class TanksController < ApplicationController
	...
	if @tank.save
      format.html { redirect_to @tank, notice: 'Tank was successfully added' }
		...
		format.json {render json: @tank.errors, status: :unprocessable_entity }
	      ...
	      @tank.errors.add(:base, e.message.split('Mysql2::Error: ')[1].strip.split(': INSERT')[0].strip)
	      format.html {render :new }
	      format.json {render json: @tank.errors, status: :unprocessable_entity}
      end
    end
	...
	if @tank.update(tank_params)
      format.html { redirect_to @tank, notice: 'Tank was successfully updated' }
		...
		format.json {render json: @tank.errors, status: :unprocessable_entity }
	     ...
	     @tank.errors.add(:base, e.message.split('Mysql2::Error: ')[1].strip.split(': UPDATE')[0].strip)
	      format.html {render :edit }
	      format.json {render json: @tank.errors, status: :unprocessable_entity}
      end
    end
  ...
  private
    ...
    def tank_params
      params.require(:tank).permit(:tank_id, :name, :description, :water_type, :volume, :temperature, :year_built, :habitat_id)
    end
end
